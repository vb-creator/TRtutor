...
3. <conversation_history>  
   • JSON list of prior turns until the last student prompt
...
def make_inference_prompt(
    ...
    persona_json  = json.dumps(student_persona, ensure_ascii=False)
    reasoner_json = json.dumps(reasoner_context, ensure_ascii=False)
    history_json  = json.dumps(conversation_history, ensure_ascii=False)

    # fill in the this exactly as in your SFT formatting
    prompt = TALKER_PROMPT.format(
        STUDENT_PERSONA      = persona_json,
        QUESTION             = question,
        REASONER_CONTEXT     = reasoner_json,
        CONVERSATION_HISTORY = history_json,
    )

    # append EOS so the model knows to stop when it generates it
...
class Talker:
    ...
    def __init__(self, model_path: str, prompt_template: str):
        ...
        model_path = "finetuned_models/finetuned_talker_v4/"
        ...
        offload_dir = "./offload"
        self.model = PeftModel.from_pretrained(model, model_path, torch_dtype=torch.float16, offload_folder=offload_dir)
        self.model.eval()
    ...
    def respond(self, question, student_persona, reasoner_context, conversation_history):
        formatted_prompt = make_inference_prompt(
        question = question,
        ...
        with torch.no_grad():
            ...
            # new_ids = gen[0, tokens["input_ids"].size(-1):]
            new_ids = gen[0][tokens["input_ids"].shape[-1]:]
            # print("gen shape:", gen.shape)
            # print("raw token ids:", gen[0].tolist()[: tokens["input_ids"].size(-1) + 5])
            # print("decoded full:", self.tokenizer.decode(gen[0], skip_special_tokens=False))
        
        # print(f"input length: {tokens["input_ids"].size(-1)} vs max:", self.model.config.max_position_embeddings)
...
class Tutor:
    ...
    def __init__(self, talker, reasoner):
        self.talker = talker
        self.reasoner = reasoner

    def setter(self, question, student_persona, reasoner_context, conversation_history = []):
        self.question = question
        self.student_persona = student_persona
        self.reasoner_context = reasoner_context
    ...
    def teacher_respond(self) -> str:
        reply = self.talker.respond(self.question, self.student_persona, self.reasoner_context, self.conversation_history)
...
talker = Talker(model_path=file_path, prompt_template=TALKER_PROMPT)
reasoner = Reasoner(api_key=OPENAI_API_KEY,
                        prompt_template=REASONER_PROMPT)
tutor = Tutor(talker=talker, reasoner=reasoner)

# EOS_TOKEN = talker.tokenizer.eos_token_id
test_json = pd.read_json("tr_data/test_mathdial.json")

test_filtered_df = test_json.drop_duplicates(['question', 'student_persona_str'])

from tqdm import tqdm
conversations_generated = []
first_200_rows = test_filtered_df[:200].to_dict("records")
...
def simulate_conversation(tutor: Tutor, question: str, student_persona: dict, student_model):
    ...
    while tutor.turn_counter < 5:
        # 1) get model output via Talker.respond
        # teacher_reply = tutor.talker.respond(tutor.question,
        #     tutor.student_persona,
        #     tutor.reasoner_context,
        #     tutor.conversation_history
        # )
        teacher_reply = tutor.teacher_respond()
        print(f"Teacher: {teacher_reply}")
        tutor.conversation_history.append({"speaker": "teacher", "text": teacher_reply})
        ...
        print(f"Student: {student_prompt}")
        tutor.conversation_history.append({'speaker': 'student', 'text': student_prompt})
        conversation_history.append({'speaker': 'student', 'text': student_prompt})
...
for index, row in tqdm(enumerate(first_200_rows)):
    ...
    tutor.setter(question=question, student_persona=student_persona, reasoner_context={})
    print("-" * 50)
    ...
    if index % 5 == 0:
        ...
        conversations_generated_df.to_json("tr_data/conversations_generated_llama_7b_df.json")